...
- **No standalone transformer encoder** — features go straight from the
  conv-based projector into the two-stage decoder init
...
- **Shared single class/bbox heads** at top level (one Linear + one MLP),
  not per-decoder-layer copies
...
| `rfdetr.backbone.pos_embed_train_size`       | uint32      | `37` (side length of the stored positional grid; runtime image of 560/14=40 patches is bilinearly interpolated from 37×37+1) |
| `rfdetr.projector.in_dim`                    | uint32      | `1536` (= 4 × backbone.dim) |
...
(`= num_queries × group_detr = 300 × 13`). Only group 0 (the first 300 rows)
...
Per-block: 18 tensors. Total backbone: `4 + 18 × 12 + 2 = 222`.
...
(`4 × 384 = 1536` channels) and emits a single 256-channel feature map.
...
| `projector.cv2.conv.weight`                 | `(1, 1, 640, 256)`  | `…stages.0.0.cv2.conv.weight` (640 = 256 + 128×3 after C2f split+bottleneck concat) |
...
Per group: 12 tensors. Total: `13 × 12 = 156`.
...
- **Cross-attention** — `MSDeformAttn` (deformable, single-scale):
  16 heads, 1 level, 2 sampling points per head
  - `sampling_offsets: Linear(256 → 64)` (`64 = 2 × heads × n_levels × n_points = 2 × 16 × 1 × 2`)
  - `attention_weights: Linear(256 → 32)` (`32 = heads × n_levels × n_points = 16 × 1 × 2`)
  - `value_proj: Linear(256 → 256)`
  - `output_proj: Linear(256 → 256)`
- **FFN** — `linear1: 256 → 2048`, `linear2: 2048 → 256`
- **3 LayerNorms** — `norm1` (post self-attn), `norm2` (post cross-attn), `norm3` (post FFN)

Plus shared decoder-level state:

- `decoder.norm` — final LayerNorm(256)
- `decoder.ref_point_head` — 2-layer MLP `(512 → 256 → 256)`, projects sinusoidally
  embedded 4D reference points (cx, cy, w, h × 128 freq → 512) down to 256
...
Per decoder layer: 22 tensors. Total decoder: `2 + 4 + 22 × 3 + 2 = 74`.

### Heads — 8 tensors

Both heads are **shared single instances** at the top of the model (the
upstream calls these `inner.class_embed` and `inner.bbox_embed`). There is
**no per-decoder-layer head**.
