...
(PyTorch Lightning) — see [TrainConfig in rfdetr/config.py](https://github.com/roboflow/rf-detr/blob/main/rfdetr/config.py)
...
in the `.pth` checkpoint; we'd be guessing a schema if we embedded them.)
...
Q8_0 gives ~3.1x smaller files with no measurable accuracy loss on the COCO
benchmark — see [BENCHMARK.md](../BENCHMARK.md). For Q4 variants there are
...
from rfdetr.cpp on the same image:
...
Detections should match to sub-pixel precision — rfdetr.cpp's parity
...
- **Dataset preprocessing in C++** — rfdetr.cpp accepts a JPEG/PNG path and
  applies the standard ImageNet normalize internally; data augmentation
  happens during training in Python.
- **Segmentation fine-tunes** — the converter is detection-only today. A
  segmentation conversion path is planned (see the `docs/superpowers/plans/`
  directory for the in-progress Plan C).
