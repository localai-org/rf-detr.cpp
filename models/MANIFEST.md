...
The `.gguf` files themselves are **gitignored**; this manifest is the
canonical record of what should exist and at what size. To rebuild from
scratch:

```bash
# 1. Convert PyTorch -> F32 GGUF (one per variant)
for v in nano small base medium large seg-nano seg-small seg-medium; do
    scripts/convert_rfdetr_to_gguf.py --variant "$v" --output "models/rfdetr-${v}-f32.gguf"
done
...
All 32 models have been verified to load and run `rfdetr-cli detect` on
...
- **F32** — full precision reference, ~120 MB per variant.
...
- **Q8_0** — best accuracy/size trade for production; ~3x smaller than F32.
- **Q4_K** — smallest practical quant; rows with `ne[0] % 256 != 0` (the
  ...
  the C++ quantizer's logic. Net result is still ~3.8-4.0x compression.
