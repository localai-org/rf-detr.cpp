...
extern "C" rfdetr_status rfdetr_preprocess(const rfdetr_image* img,
    ...
    /* Allocate the normalized F32 NCHW output, then use the resize path
     * selected by GGUF metadata.
     *
     * RF-DETR 1.9 explicitly uses torchvision F.resize(..., antialias=False)
     * for inference so it matches the cv2.INTER_LINEAR training pipeline.
     * That is bilinear interpolation with align_corners=false (half-pixel
     * source coordinates) and no intermediate uint8 rounding. stb's easy
     * resize API instead selects Mitchell/cubic filters and rounds back to
     * uint8, which materially shifts borderline detection scores. That stb
     * path remains available for GGUFs created before the metadata was added.
     *
    ...
    if (bilinear_no_antialias) {
        ...
        for (int h = 0; h < target_h; ++h) {
            ...
            const int y1 = std::clamp(y1_raw, 0, img->height - 1);
            ...
            for (int w = 0; w < target_w; ++w) {
                ...
                const int x1 = std::clamp(x1_raw, 0, img->width - 1);
