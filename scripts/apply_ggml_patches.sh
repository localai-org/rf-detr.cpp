...
# CMake configure in parallel against the same `sources/rt-detr.cpp/` clone,
# which makes the patch loop below race: process A's `git apply --check` can
# succeed before process B has applied the same patch, and then A's actual
# `git apply` fails because the hunk is already in the tree. A best-effort
# flock on a sentinel file alongside the submodule serialises that window;
# we re-exec the script under flock so the rest of the body runs serially.
if [[ -z "${RFDETR_PATCH_FLOCK_HELD:-}" ]] && command -v flock >/dev/null 2>&1; then
    LOCK_FILE="${PROJECT_ROOT}/third_party/.ggml-patch.lock"
    : > "${LOCK_FILE}" 2>/dev/null || true
    if [[ -e "${LOCK_FILE}" ]]; then
        export RFDETR_PATCH_FLOCK_HELD=1
